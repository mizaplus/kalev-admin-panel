...
export default function HomePage() {
  ...
  const [initialHero, setInitialHero] = useState<HomePageHero | null>(null);
  ...
  useEffect(() => {
    ...
    fetchHomePage()
      .then((data) => {
        ...
        setHomeData(data);
        setInitialHero(data.hero);
        form.reset(data.hero);
        setError(null);
      })
      .catch((err: unknown) => {
        if (!isMounted) {
  ...
  const previewHero = useMemo<HomePageHero>(() => {
    return {
      title:
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      setSaveMessage(
        "Hero content saved locally. Connect the update API to persist these changes.",
      );
      setError(null);
      setInitialHero(values);
      setHomeData((current) =>
        current ? { ...current, hero: values } : current,
      );
      form.reset(values);
    } catch (submitError) {
      setError(
    ...
    return [
      {
        ...
        detail: getSnippet(homeData.hero.tagline),
        status: "Editable",
        editable: true,
      },
      {
        ...
        detail: getSnippet(homeData.about.content),
        status: "Preview only",
        editable: false,
      },
      {
        ...
        detail: `${homeData.choose_us.reasons.filter((reason) => !reason.disabled).length} active reasons`,
        status: "Preview only",
        editable: false,
      },
      {
        ...
        detail: `${homeData.programs.items.length} published programs`,
  ...
  const renderSectionPreview = useCallback(() => {
    ...
    if (selectedSection === "choose_us") {
      const choose = homeData.choose_us;
      return (
        <Card>
          <CardHeader>
  ...
  const renderEditContent = () => {
    ...
    return (
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        {saveMessage ? (
          <Alert>
            <AlertTitle>Changes stored</AlertTitle>
